...
| **Broker T&C forbid fully automated order placement** — Kite Connect terms §2(e) say the APIs are "not meant for placing fully automated trades (without manual intervention)" and point to exchange approval for anything more. SEBI's below-10-OPS permission does not amend a contract with the broker. | **H** | **H** | M8.2 memo Q1 (`ops/compliance/sebi-algo-memo.md`). Interim posture costs nothing and is already built in: M8.1 mandates an explicit live-confirm flag, so real-money days are a staged order set a human releases — not "without manual intervention" on any reading. Written clarification from Zerodha before M8.3 becomes the compliance artefact for the graduation packet. **Note the tension with §0:** for real money this puts a human action in the daily loop, at the *execution* step only — policy discretion still sits entirely in the ratified-in-advance layer, and paper mode is unaffected. If the clarification comes back permissive, the confirm flag becomes a config default rather than a design change. |
...
| 2026-08-08 | — | **RATIFIED** (proposed at M0.1, ratified by owner 2026-08-08): §8.2 System 1 package `platform/` → `dataplatform/` | `platform` is a stdlib module name, so it cannot be a top-level package: after anything imports the stdlib module (pytest does) `platform.config` raises *'platform' is not a package*, and when the local package wins the path race `import pandas` fails on `platform.python_implementation()`. Both reproduced at M0.1. Only the directory name changes — module boundaries, IDs and ownership per §8.2 are untouched, and no §1 decision is affected. All 71 `platform/…` path strings in TASK_GRAPH.yaml were swept to `dataplatform/…` on ratification, so no agent has to resolve the old name from a footnote. §8.2's diagram still reads `platform/`; that block is the historical text, and this row is the correction. |
| 2026-08-08 | — | Added task M5.15: broker session re-authentication interlock (`AUTH_REQUIRED`) | M8.2's compliance research found that regulation forces a daily broker API logout with OAuth + 2FA, so the daily loop can be authenticated-invalid at market open. §4.4's interlock covers red data but not dead auth — the same class of failure (act on an invalid precondition) with no coverage. Owner chose to build it inside M5's daily loop rather than retrofit it at M8. No §1 decision affected. |

| 2026-09-04 | — | **PROPOSED** (at M11.1, awaiting owner ratification): add §4.1 row 17 "Macro / economic backdrop" | The plan has no row for the economic and market-state backdrop the analyst reasons against, so the daily index valuation series (P/E, P/B, dividend yield per index — measured back to 2012-10-01 on the already-VERIFIED `nifty_index_close_snapshot`) and any macro series have no §4.1 home and cannot be registered. Two entries are staged under the new row: `worldbank_indicator_api` (VERIFIED, annual, current-vintage only) and `alfred_series_vintage` (FAILED — unreachable from this host, cause undetermined). No §1 decision is affected; this adds a data row, changes no invariant, and nothing outside `dataplatform/ingest/macro` reads it yet. Evidence: `ops/gates/M11-macro-source-probe.md`. |

| 2026-09-08 | — | **PROPOSED** (at the OPS daily snapshotter, awaiting owner ratification): add §4.1 row 18 "Market-structure snapshots" | The plan has no row for the exchange's own daily view of a security — its industry assignment, its price band, and its surveillance framework — so `nse_industry_classification`, `nse_price_bands`, `nse_asm_list`, `nse_gsm_list` and `nse_esm_list` had no §4.1 home and could not be registered at all. All five are snapshot-only: no date parameter, no archive host, no historical form, so their history is destroyed at a rate of one day per day and an unregistered day is unrecoverable. Classification is the load-bearing one — a thematic fund's universe rule is a point-in-time classification, and until this row the repo's only sector source was a 42-ISIN fixture CSV applied backward. Each entry is VERIFIED against a live probe and archive-only (`parser: {id: null, task: null}`, a state this amendment also adds to the register schema, validated all-or-nothing). No §1 decision is affected: this adds a data row, changes no invariant, and nothing outside `dataplatform/ingest/daily_snapshot` reads it yet. Evidence: `ops/gates/daily-snapshotter-2026-09-08.md`. |
